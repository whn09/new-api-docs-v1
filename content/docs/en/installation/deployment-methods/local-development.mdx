...
This document provides detailed steps for setting up and developing the New API project in a local environment, suitable for developers who wish to contribute to the project or perform secondary development.
...
<Callout type="info" title="Hint">
  If `SQL_DSN` is not configured, the system will default to using an SQLite database, stored in the `one-api.db` file.
</Callout>
