...
<Callout type="info" title="About Bun">
  Bun is an ultra-fast JavaScript package manager, test runner, and bundler.
  Compared to traditional npm or yarn, Bun's installation speed is 25x faster,
  making it the most recommended JavaScript package management tool in 2024.
</Callout>
...
<Callout type="info" title="Note">
  If `SQL_DSN` is not configured, the system will use SQLite database by
  default, stored in the `one-api.db` file.
</Callout>
