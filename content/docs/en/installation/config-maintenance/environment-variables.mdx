...
<Callout type="info" title="Tip">
  New API supports reading environment variables from a `.env` file. Please
  refer to the `.env.example` file and rename it to `.env` for use.
</Callout>
...
<Callout type="info" title="Cluster Deployment">
  For information on how to use these environment variables to build a complete
  cluster deployment, please refer to the [Cluster Deployment
  Guide](/en/docs/installation/deployment-methods/cluster-deployment).
</Callout>
...
<Callout type="warn" title="RELAY_TIMEOUT Setting Warning">
  Please be cautious when setting the `RELAY_TIMEOUT` environment variable.
  Setting it too short may lead to the following issues:
</Callout>
...
<Callout type="info" title="Complete Cluster Configuration">
  This is just a basic multi-node configuration example. For complete cluster
  deployment configuration, architectural descriptions, and best practices,
  please refer to the [Cluster Deployment
  Guide](/en/docs/installation/deployment-methods/cluster-deployment).
</Callout>
